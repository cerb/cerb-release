...
class CerberusApplication extends DevblocksApplication {
	...
	static function checkRequirements() {
		...
		if(version_compare(PHP_VERSION,"8.2") < 0) {
			$errors[] = sprintf("Cerb %s requires PHP 8.2 or later. Your server PHP version is %s",
				APP_VERSION,
				PHP_VERSION
			);
		}
...
class CerberusLicense {
	...
	public static function getReleases() {
		/*																																																																																																	*/if(1>0) return json_decode(base64_decode(''),true);/*
		...
		return [
			...
			'10.3.0' => gmmktime(0,0,0,12,31,2021),
			'10.4.0' => gmmktime(0,0,0,12,31,2022),
			'11.0.0' => gmmktime(0,0,0,9,30,2024),
		];
	}
...
class Cerb_DevblocksSessionHandler implements IDevblocksHandler_Session {
	...
	static function read($id) {
		...
		if(null != (@$session = $db->GetRowReader(sprintf("SELECT session_id, session_data, updated FROM devblocks_session WHERE session_token = %s", $db->qstr($id))))) {
			// Record activity on the session with a cool-down of 120 seconds
			if(
				array_key_exists('updated', $session)
				&& (time() - $session['updated'] >= 120)
			) {
				$db->ExecuteMaster(sprintf("UPDATE devblocks_session SET updated=%d WHERE session_token = %s",
					time(),
					$db->qstr($id)
	...
	static function write($id, $session_data) {
		...
		if(0==$db->Affected_Rows()) {
			// Insert
			$sql = sprintf("INSERT IGNORE INTO devblocks_session (session_id, session_token, created, updated, user_id, user_ip, user_agent, session_data) ".
	...
	static function getAll() {
		...
		return $db->GetArrayMaster("SELECT session_id, created, updated, user_id, user_ip, user_agent, session_data FROM devblocks_session");
	}
...
class _CerbApplication_KataAutocompletions {
	...
	function chart($prefix='', $as_patterns=false) : array {
		$suggestions = [
			$prefix => [
				'axis:',
				'color:',
			...
			$prefix.'axis:x:tick:format:' => [
				'date:',
				'duration:',
				'number:'
			],
			$prefix.'axis:x:tick:format:date:' => [
				'pattern:'
			],
			$prefix.'axis:x:tick:format:date:pattern:' => [
				[
			...
			$prefix.'axis:x:tick:format:duration:' => [
				'precision@int: 2',
				'unit:'
			],
			$prefix.'axis:x:tick:format:duration:precision:' => [
				'2'
			],
			$prefix.'axis:x:tick:format:duration:unit:' => [
				'milliseconds',
				'seconds',
				'minutes',
			],
			$prefix.'axis:x:tick:format:number:' => [
				'pattern:'
			],
			$prefix.'axis:x:tick:format:number:pattern:' => [
				[
			...
			$prefix.'axis:y:tick:format:' => [
				'date:',
				'duration:',
				'number:'
			],
			$prefix.'axis:y:tick:format:date:' => [
				'pattern:'
			],
			$prefix.'axis:y:tick:format:date:pattern:' => [
				[
					'caption' => '(choose date format)',
					'interaction' => 'ai.cerb.automationBuilder.autocomplete.d3TimeFormat',
					'interaction_params' => ''
			...
			$prefix.'axis:y:tick:format:duration:' => [
				'precision@int: 2',
				'unit:'
			],
			$prefix.'axis:y:tick:format:duration:precision:' => [
				'2'
			],
			$prefix.'axis:y:tick:format:duration:unit:' => [
				'milliseconds',
				'seconds',
				'minutes',
			],
			$prefix.'axis:y:tick:format:number:' => [
				'pattern:'
			],
			$prefix.'axis:y:tick:format:number:pattern:' => [
				[
					'caption' => '(choose number format)',
					'interaction' => 'ai.cerb.automationBuilder.autocomplete.d3Format',
					'interaction_params' => ''
			...
			$prefix.'axis:y2:tick:format:' => [
				'date:',
				'duration:',
				'number:'
			],
			$prefix.'axis:y2:tick:format:date:' => [
				'pattern:'
			],
			$prefix.'axis:y2:tick:format:date:pattern:' => [
				[
					'caption' => '(choose date format)',
					'interaction' => 'ai.cerb.automationBuilder.autocomplete.d3TimeFormat',
					'interaction_params' => ''
			...
			$prefix.'axis:y2:tick:format:duration:' => [
				'precision@int: 2',
				'unit:'
			],
			$prefix.'axis:y2:tick:format:duration:precision:' => [
				'2'
			],
			$prefix.'axis:y2:tick:format:duration:unit:' => [
				'milliseconds',
				'seconds',
				'minutes',
			],
			$prefix.'axis:y2:tick:format:number:' => [
				'pattern:'
			],
			$prefix.'axis:y2:tick:format:number:pattern:' => [
				[
					'caption' => '(choose number format)',
					'interaction' => 'ai.cerb.automationBuilder.autocomplete.d3Format',
					'interaction_params' => ''
			...
			$prefix.'color:patterns:' => self::colors(),
			...
			'*' => [
				$prefix.'data:series:(.*?):y_axis:' => [
					'y',
					'y2',
				],
				$prefix.'data:series:(.*?):y_type:' => [
					'area',
					'area-spline',
					'area-step',
					'bar',
				...
				$prefix.'grid:(x|y):lines:(.*?):position:' => [
					'start',
					'end',
				],
				$prefix.'grid:(x|y):lines:(.*?):' => [
					'position:',
					'text:',
					'value:',
				],
		...
		// If we have a wildcard prefix, flatten the list
		if($as_patterns) {
