...
class DAO_Ticket extends Cerb_ORMHelper {
	...
	public static function updateWithMessageProperties(array &$properties, Model_Ticket &$ticket, array $change_fields=[], $unset=true) {
		...
		if(array_key_exists('status_id', $properties) && !is_null($properties['status_id'])) {
			$status_id = DevblocksPlatform::importVar($properties['status_id'] ?? null, 'int', 0);
			$reopen_at = DevblocksPlatform::importVar($properties['ticket_reopen'] ?? null, 'string', '');
	...
	static function processUpdateEvents($ids, $change_fields) {
		...
		foreach($models as $id => $model) { /* @var $model Model_Ticket */
			...
			$status_id = $change_fields[DAO_Ticket::STATUS_ID] ?? null;
	...
	static function autocomplete($term, $as='models', $query=null) {
		...
		$view = $context_ext->getSearchView('autocomplete_tickets');
		$view->is_ephemeral = true;
		$view->renderPage = 0;
		$view->renderSortBy = SearchFields_Ticket::TICKET_ID;
		$view->renderSortAsc = false;
		$view->addParamsWithQuickSearch($query, true);
		
		$view->addParamsWithQuickSearch('(mask:${term} OR subject:${term})', false, [
			'term' => $term . '*',
		]);
		...
		return match ($as) {
			...
			default => DAO_Ticket::getIds(array_keys($results)),
		};
	}
	...
	static function getWhereSQL(DevblocksSearchCriteria $param) {
		switch($param->field) {
			...
			case self::VIRTUAL_HAS_FIELDSET:
				return self::_getWhereSQLFromVirtualSearchSqlField($param, CerberusContexts::CONTEXT_CUSTOM_FIELDSET, sprintf('SELECT context_id FROM context_to_custom_fieldset WHERE context = %s AND custom_fieldset_id IN (%s)', Cerb_ORMHelper::qstr(CerberusContexts::CONTEXT_TICKET), '%s'), 't.id');
...
class Context_Ticket extends Extension_DevblocksContext implements IDevblocksContextPeek, IDevblocksContextProfile, IDevblocksContextImport, IDevblocksContextMerge, IDevblocksContextAutocomplete, IDevblocksContextBroadcast {
	...
	function autocomplete($term, $query=null) {
		// Allow per-instance overrides for the default autocomplete filtering/sort
		if(APP_OPT_AUTOCOMPLETE_TICKET_QUERY && !$query)
			$query = APP_OPT_AUTOCOMPLETE_TICKET_QUERY;
		
		if(!($results = DAO_Ticket::autocomplete($term, 'models', $query)))
			return [];
		
		$list = [];

		$dicts = DevblocksDictionaryDelegate::getDictionariesFromModels($results, Context_Ticket::ID, ['group_', 'initial_message_sender_']);
